...
/* AXI4HyperSpace */
class HyperSpaceGOTester[T <: Data : Real: BinaryRepresentation]
(
  ...
  if(params.cfarParams != None) {
    val beatBytes = 4
    val cfarModeString = "Greatest Of"
  ...
  if (params.cfarParams != None) {
    ...
    // Plot data
    if (enablePlot) {
      HyperSpaceTesterUtils.plot_data(inputData = outCUT, plotName = "MAgnitude", fileName = "./AXI4HyperSpaceGO/plot_mag.pdf")
      HyperSpaceTesterUtils.plot_data(inputData = fftMagScala, plotName = "MAgnitude", fileName = "./AXI4HyperSpaceGO/plot_mag_scala.pdf")
    }
  }
  else {
    ...
    if (enablePlot) {
      HyperSpaceTesterUtils.plot_data(inputData = testTone.map(m => m * math.pow(2,14)).map(c => c.real.toInt), plotName = "Input real data without reordering", fileName = "AXI4HyperSpaceGO/in_real.pdf")
      HyperSpaceTesterUtils.plot_data(inputData = testTone.map(m => m * math.pow(2,14)).map(c => c.imag.toInt), plotName = "Input imag data without reordering", fileName = "AXI4HyperSpaceGO/in_imag.pdf")
      HyperSpaceTesterUtils.plot_data(inputData = input.map(c => c.real.toInt), plotName = "Input real data with reordering", fileName = "AXI4HyperSpaceGO/in_real_reordered.pdf")
      HyperSpaceTesterUtils.plot_data(inputData = input.map(c => c.imag.toInt), plotName = "Input imag data with reordering", fileName = "AXI4HyperSpaceGO/in_imag_reordered.pdf")
    }

    // Write data to text files
    val f_output = new File("./test_run_dir/AXI4HyperSpaceGO/output_data.txt")
    ...
    val f_input = new File("./test_run_dir/AXI4HyperSpaceGO/input_data.txt")
