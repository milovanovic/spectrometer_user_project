...
//  Chain Test
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  it should "test AXI4HyperSpace with Cell Averaging" in {
  ...
  it should "test AXI4HyperSpace with Greatest Of" in {
    val lazyDut = LazyModule(new AXI4HyperSpace(params, 4) with AXI4HyperSpacePins)
    chisel3.iotesters.Driver.execute(Array("--backend-name", simulator, "--target-dir", "test_run_dir/AXI4HyperSpaceGO", "--top-name", "AXI4HyperSpace"), () => lazyDut.module) {
      c => new HyperSpaceGOTester(lazyDut, params, fftSize.toInt, false, enablePlot.toBoolean, true)
    } should be (true)
  }

  it should "test AXI4HyperSpace with Smallest Of" in {
    val lazyDut = LazyModule(new AXI4HyperSpace(params, 4) with AXI4HyperSpacePins)
    chisel3.iotesters.Driver.execute(Array("--backend-name", simulator, "--target-dir", "test_run_dir/AXI4HyperSpaceSO", "--top-name", "AXI4HyperSpace"), () => lazyDut.module) {
      c => new HyperSpaceSOTester(lazyDut, params, fftSize.toInt, false, enablePlot.toBoolean, true)
    } should be (true)
  }
}
